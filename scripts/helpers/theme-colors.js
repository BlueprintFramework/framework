...
function printlog(message, level) {
  if(process.env.BLUEPRINT_DEBUG) {
    if(level == "log" || !level) {
      console.log(`blueprint-theme-colors: ${message}`)
    } else if (level == "warn") {
      console.warn(`blueprint-theme-colors: (warn) ${message}`)
    } else if (level == "error") {
      console.error(`blueprint-theme-colors: (error) ${message}`)
    }
  }
}
...
function generateColorsFromCss(cssFilePath) {
  ...
  printlog(`found ${Object.keys(cssVars).length} blueprint variables`);
  ...
  printlog(`generated ${Object.keys(colors).length} color groups:`, Object.keys(colors).sort());
  return colors;
}
...
function extractAllCssVars(cssFilePath, processedFiles = new Set()) {
  ...
  if (!fs.existsSync(cssFilePath)) {
    printlog(`file not found: ${cssFilePath}`, 'warn');
    return {};
  }
...
module.exports = { generateColorsFromCss };
